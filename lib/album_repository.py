...
class AlbumRepository:
    ...
    def add_album(self, title, release_year, artist_id):
    ...
    def get_album_with_artist(self, album_id):
        [row] = self._connection.execute('SELECT albums.id, albums.title, albums.release_year, albums.artist_id, artists.id AS artists_id, artists.name, artists.genre FROM albums JOIN artists ON albums.artist_id = artists.id WHERE albums.id = %s', [album_id])
